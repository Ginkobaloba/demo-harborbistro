...
## D-021: Label the portal-handoff machinery as unreached, do not wire it (2026-09-19)
...
`handler.ts`), the client handoff page (`src/app/portal/handoff/page.tsx`),
and `readHarborSession` (`src/lib/portal-session.ts`) are complete, tested
machinery that is never exercised in production. Two independent
observations agreed on this tonight:

- The Opus deep verify of PR #37 (`verify/reports/DEEP_VERIFY_2026-09-19_pr37-customer-session-claims.md`)
  found `readHarborSession` has zero callers anywhere in the repo, static
  analysis and grep.
- The portal owner (the session that owns portal-shell) reports that
  harborbistro is a `shape: "iframe"` tile in the portal, the iframe path
  renders the frame with no fragment, and so the portal never navigates a
  visitor to `/portal/handoff#portal_token=...` at all. This route and
  page have no live caller from the portal side, independent of anything
  in this repo.
...
tiles receive a token, or the portal moves harborbistro to a top-level
...
## D-023: The proxy-level containment, written down at last (2026-09-25)

The `/admin` 404 that harbor has served publicly since 2026-09-18 had **no
decision record**. D-022's brief called it D-017; D-017 is checkout ordering
and body caps. So the control that was doing *all* of the work here was the
only one nobody had documented. This entry fixes that, and corrects two
things people have assumed about it.

**What it is.** `demo-proxy` returns a hard 404, before the request ever
reaches this app, for five route families:

    location ^~ /admin                { return 404; }
    location ^~ /api/admin            { return 404; }
    location ^~ /order/confirmation/  { return 404; }
    location ^~ /api/orders/          { return 404; }
    location ~ ^/reservations/[^/]+   { return 404; }

Added as an emergency on 2026-09-18 ("fix the exposure immediately"): the
admin views showed visitor-entered names, emails and phone numbers, and the
public detail routes were keyed by a guessable 5-symbol code that revealed
name, notes, party size and delivery address.

**Where it lives, and why it survives a redeploy.** In
`cloudflare-config/nginx/conf.d/extra/harborbistro.locations`, which **is**
tracked. The per-demo server block (`nginx/conf.d/harborbistro.conf`) is
generated by Terraform and gitignored on purpose, so the rules were put in a
tracked file that the template pulls in with
`include /etc/nginx/conf.d/extra/${name}.locations*;`. A re-render therefore
cannot drop the containment. The generated file also still carries an inline
copy from the emergency; the two agree today, and whoever removes one must
remove both.

**Correction 1: this is not a harbor-only control.** slatewell carries the
identical containment.

**Correction 2, and the one that matters: D-022 does NOT cover all of it.**
The app gate added in D-022 covers `/admin` and `/api/admin` only. The other
three families have **no app-level 404** -- they are protected in-app by
D-016 visitor-scoping, which answers 404 for another visitor's record but
does not hide the routes themselves.

So **"the app gate is verified, remove the containment" is wrong as stated.**
Deleting `harborbistro.locations` wholesale would re-expose order
confirmations, the orders API and reservation detail to anyone who guesses a
5-symbol code. Removal is per-route-family, not per-file:

- `/admin`, `/api/admin`: removable once D-022's gate is verified **at the
  container port** -- not at the public hostname, which returns 404 either way
  and so cannot tell a working gate from a missing one. See
  `verify/reports/pr50-admin-gate-live-evidence.md`.
- `/order/confirmation/`, `/api/orders/`, `/reservations/*`: need a separate
  decision about whether D-016 scoping alone is enough for a guessable id.
  Not covered by D-022 and not answered here.

**The removal condition is a comment, not a control.** The file says "remove
